...
export class Web3Contract {
  ...
  private encodeFunction(functionName: string, types: string[], values: any[]): string {
    const iface = new Interface([`function ${functionName}(${types.join(",")})`])
    const normalizedValues = values.map((value, index) => {
      if (types[index] === "uint256") {
        return BigInt(value)
      }
    ...
    return iface.encodeFunctionData(functionName, normalizedValues)
  }

  private getFunctionSignature(functionName: string, types: string[]): string {
    const signature = `${functionName}(${types.join(",")})`
    return "0x" + keccak256(toUtf8Bytes(signature)).slice(2, 10)
  }
}
